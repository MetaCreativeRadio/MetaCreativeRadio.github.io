...
**使用すべきケース**:
- コードベース全体の構造理解
- 特定パターンのファイル検索（`src/**/*.tsx`）
- キーワード検索（「API endpoints」「error handling」）
- アーキテクチャ分析
...
```
「エラーハンドリングの実装箇所を調査」
→ Exploreエージェント（medium）を使用
```

#### 3. Definition of Done (DoD) フェーズ管理

**各フェーズ完了時の必須チェックリスト**:

1. ✅ **コードレビュー**
   - ESLint/型チェックエラーなし
   - セキュリティ脆弱性チェック（XSS、SQL injection、command injection等）
   - コーディング規約遵守確認
...
| タスク種別 | 推奨エージェント | 理由 |
|----------|----------------|------|
| コードベース探索 | Explore (medium) | 効率的な全体把握 |
| 複数ファイル処理 | general-purpose（並列） | 同時処理で高速化 |
| 文字起こし処理 | general-purpose（並列） | エピソード数×並列度 |
| UI実装 | general-purpose | 単一コンポーネント集中 |
| バグ調査 | Explore (thorough) | 徹底的な原因特定 |
...
```
❌ 従来: 5エピソード × 10分 = 50分
✅ 並列: 5エピソード並列実行 = 12分（4倍高速）
```

**例2: コードベース全体のリファクタリング**
```
Phase 1: Explore（medium）でアーキテクチャ把握（3分）
Phase 2: 5ファイルを並列リファクタリング（8分）
Phase 3: DoD完全実施（5分）
合計: 16分（逐次なら40分）
```
...
**ビルドエラー発生時**:
1. エラー内容を正確に把握
2. Exploreで関連コード調査（thorough）
3. 原因特定後、修正実施
4. 再度DoD完全実施

**デプロイ失敗時**:
1. GitHub Actions履歴確認
2. ローカルで `npm run build` 再実行
3. エラーログ詳細分析
4. 修正後、DoD完全実施して再push
...
1. **第8話Amazon Music URL追加**
   - URL: https://music.amazon.co.jp/podcasts/0b61adb0-4ff9-40d5-9bd9-88b834536c04/episodes/079b3cd5-afef-412a-a121-22ced3e96c44/8備蓄から紐解くクリエイターの生活ロジック八丈島台風被災後編
   - 自動RSS更新で取得済みのSpotify/Apple Podcasts URLに追加

2. **デプロイ完了**
   - ビルド完了、GitHub Pagesへプッシュ済み
   - 第8話が全プラットフォーム対応で公開

### 📊 現在の状況

**公開済みエピソード**: 8話（全て全プラットフォーム対応完了）
- 第1話〜第8話: Spotify ✅ / Apple Podcasts ✅ / Amazon Music ✅
...
3. **public/data/episodes.json**
   - Episode 7: Spotify URL更新、Amazon Music URL追加
   - Episode 8: Amazon Music URL追加（Apple/Spotifyは自動取得済み）
